...
export const CompetitionDetailsStep: React.FC<StepComponentProps> = ({
  ...
  const isAnyWeightClassSelected = data.weightClasses.includes('Any');
  ...
  const handleAnyWeightClassChange = () => {
    if (isAnyWeightClassSelected) {
      ...
      updateData({
        weightClasses: data.weightClasses.filter(wc => wc !== 'Any')
      });
    } else {
  ...
  return (
    <div className="space-y-8">
      ...
      <div>
        ...
        <div className={`grid grid-cols-1 md:grid-cols-2 gap-6 ${isAnyWeightClassSelected ? 'opacity-50' : ''}`}>
        ...
        {/* Custom Weight Class - Only show if "Any" is not selected */}
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4 text-sm">
          <div>
            ...
            <p className="text-white font-medium">
              {isAnyWeightClassSelected ? 'Any' : `${data.weightClasses.length} selected`}
            </p>
          </div>
          <div>
