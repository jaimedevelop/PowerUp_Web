...
import { MeetListItem, MeetStatus, OverallStats } from '../../types/meets';

interface MeetsListProps {
  onCreateMeet: () => void;
}

export const MeetsList: React.FC<MeetsListProps> = ({ onCreateMeet }) => {
  ...
  const loadMeets = async () => {
    try {
      ...
      const transformedMeets: MeetListItem[] = fetchedMeets.map(meet => ({
        ...
        name: meet.name,
        date: meet.date,
        location: meet.location, // Full location object
  ...
  const filteredMeets = meets.filter(meet => {
    const matchesSearch = 
      ...
      meet.location.city.toLowerCase().includes(searchTerm.toLowerCase()) ||
      meet.location.state.toLowerCase().includes(searchTerm.toLowerCase()) ||
      meet.location.venue.toLowerCase().includes(searchTerm.toLowerCase());
        ...
        <Card className="p-4">
          <div className="text-center">
            <div className="text-2xl font-bold text-blue-400">
              {stats?.totalRegistrations || meets.reduce((sum, meet) => sum + meet.registrations, 0)}
            </div>
        ...
        <Card className="p-4">
          <div className="text-center">
            <div className="text-2xl font-bold text-purple-400">
              ${(stats?.totalRevenue || meets.reduce((sum, meet) => sum + meet.revenue, 0)).toLocaleString()}
            </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 xl:grid-cols-3 gap-6">
          {filteredMeets.map((meet) => (
            <MeetCard 
              ...
              meet={{
                ...meet,
                location: formatLocationString(meet.location), // Convert to string for MeetCard
              }}
              onEdit={() => {
